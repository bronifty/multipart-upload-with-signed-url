import { ListObjectsV2Command, DeleteObjectsCommand, ListObjectVersionsCommand, } from "@aws-sdk/client-s3";
import { createS3Client } from ".";
// Function to delete all objects and versions in a bucket
async function emptyBucket(profile, bucketName) {
    ...
    let objects = await s3Client.send(new ListObjectsV2Command({ Bucket: bucketName }));
    if (objects.Contents && objects.Contents.length > 0) {
        await deleteAll(objects.Contents);
    }
    let versions = await s3Client.send(new ListObjectVersionsCommand({ Bucket: bucketName }));
