...
async function multipartUploadExample(config) {
    ...
    try {
        ...
        const createMultipartUploadOutput = await s3Client.send(createMultipartUploadCommand);
        ...
        const completeMultipartUploadOutput = await s3Client.send(completeMultipartUploadCommand);
        console.log("Upload completed successfully:", completeMultipartUploadOutput);
    }
    catch (error) {
